...
public class XJarDecryptor extends XEntryDecryptor<JarArchiveEntry> implements XDecryptor, XConstants {
    ...
    @Override
    public byte[] decrypt(File src, XKey key, String clazz) throws IOException {
        ...
        try (
                ...
                XUnclosedOutputStream nos = new XUnclosedOutputStream(byteArrayOutputStream);
                XUnclosedOutputStream nos2 = new XUnclosedOutputStream(byteArrayOutputStream2);
        ) {
            JarArchiveEntry entry;
            while ((entry = zis.getNextJarEntry()) != null) {
                if (entry.getName().startsWith(XJAR_SRC_DIR)
                ...
                if (entry.isDirectory()) {
                    ...
                    if (entry.getName().equals(clazz)) {
                        try (OutputStream eos = decryptor.decrypt(key, nos2)) {
                            XKit.transfer(nis, eos);
                        }
                    } else {
                        try (OutputStream eos = decryptor.decrypt(key, nos)) {
