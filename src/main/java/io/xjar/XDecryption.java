...
public class XDecryption {
     ...
     * @throws Exception 解密异常
     */
    public byte[] toClazz(String clazz) throws Exception {
        if (clazz == null || clazz.isEmpty() || !clazz.endsWith(".class")) {
            throw new IllegalArgumentException("class类名格式错误");
        }
        if (key == null) {
            throw new IllegalArgumentException("密码错误");
        }

        return XCryptos.decrypt(xJar, key, clazz);
    }
}
